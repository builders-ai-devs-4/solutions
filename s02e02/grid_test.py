...
img  = cv2.imread(str(task_data_folder / f"{INPUT_PATH.stem}_binary_sharp2.jpg"))
...
result = cv2.bitwise_not(adapt)
cv2.imwrite(str(task_data_folder / f"{INPUT_PATH.stem}_binary_sharp2_binary.jpg"), result)
...
img  = cv2.imread(str(task_data_folder / f"{INPUT_PATH.stem}_binary_sharp2_binary.jpg"))
gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
