...
All edges between adjacent cells are consistent.
All power stations are powered. Server returns {FLG:...} → puzzle solved.
...
- If "?" appears again after two consecutive resets, log the issue and stop;
  do not attempt further rotations on an unreliable classification.
...
- Never call scan_flag on classify_grid output — only on rotate_cell responses.
